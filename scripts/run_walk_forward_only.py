...
from src.psx_predictor.models.model_factories import baseline_factory, ridge_factory, xgboost_factory, lightgbm_factory
...
def main():
    ...
    # Second tree-ensemble opinion (comparative-study literature consistently
    # shows tree ensembles as the accuracy/speed sweet spot on wide tabular
    # data vs. SVM at this row count) - same feature prep as XGBoost since
    # LightGBM also natively consumes pandas 'category' dtype for sector/ticker.
    print("Running LightGBM Walk Forward...")
    lgbm_results = run_walk_forward(lightgbm_factory, _xgb_feature_fn, windows, tickers)
    lgbm_id = uuid.uuid4().hex
    register_run(
        run_id=lgbm_id,
        model_type="lightgbm",
        feature_set_version=FEATURE_SET_VERSION,
        ticker_list=tickers,
        results_df=lgbm_results,
        model_path=os.path.join(ROOT_DIR, "models", "lightgbm_model.json"),
    )

    print("\n=== Walk-forward comparison summary (mean across all windows/tickers) ===")
    summary = {
        "baseline (random forest)": baseline_results,
        "ridge (linear baseline)": ridge_results,
        "xgboost": xgb_results,
        "lightgbm": lgbm_results,
    }
    for name, res in summary.items():
        if res.empty:
            print(f"{name:28s}: no results")
            continue
        print(
            f"{name:28s}: mae={res['mae'].mean():.4f}  rmse={res['rmse'].mean():.4f}  "
            f"dir_acc={res['directional_accuracy'].mean():.2f}%  "
            f"naive_dir_acc={res['naive_directional_accuracy'].mean():.2f}%"
        )
